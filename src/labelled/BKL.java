...
/**
 * Labelled clique algorithm simple version. Takes a labelled graph and a budget as arguments in the constructor.
 * Should be started by calling {@link algorithm.CliqueAlgorithm#execute()} instead of calling run directly.
 */
public class BKL extends CliqueAlgorithm {
	 ...
	 */
	public BKL(LabelledGraph graph, int budget, boolean verbose) {
		super(verbose);
		this.graph = graph;
		this.budget = budget;
	}
	 ...
	 * @param budget a maximum allowed number of different labels in the cliques.
	 */
	public BKL(LabelledGraph graph, int budget) {
